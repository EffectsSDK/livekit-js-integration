...
import { EffectsVideoProcessor } from './EffectsVideoProcessor';
import { LayoutMode } from './EffectsStateManagement';

const videoProcessor = new EffectsVideoProcessor();
...
const appActions = {
  ...
  applyBlur: async () => {
    ...
    videoProcessor.apply({ running: true, blur: 0.6 });
  },

  applyColor: async () => {
    videoProcessor.clear();
    videoProcessor.apply({ running: true, color: 0x00ff00 });
  },
  ...
  applyCCorrection: async () => {
    videoProcessor.clear();
    videoProcessor.apply({ running: true, ccorrection: 0.6 });
  },

  applyLowLight: async () => {
    videoProcessor.clear();
    videoProcessor.apply({ running: true, lowlight: 0.4 });
  },

  applyMirroring: async () => {
    videoProcessor.clear();
    videoProcessor.apply({ running: true, mirroring: true });
  },

  applySmartZoom: async () => {
    videoProcessor.clear();
    videoProcessor.apply({ running: true, smartzoom: 0.2 });
  },

  applyBeautification: async () => {
    videoProcessor.clear();
    videoProcessor.apply({ running: true, beautification: 0.8 });
  },

  applySharpness: async () => {
    videoProcessor.clear();
    videoProcessor.apply({ running: true, sharpness: 0.5 });
  },
